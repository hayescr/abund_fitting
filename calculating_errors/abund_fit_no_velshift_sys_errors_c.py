...
element_list = ['Fe', 'Mg', 'Ti', 'Ca', 'Ni', 'Cr', 'Na', 'K', 'Sc', 'Mn', 'Y', 'Ba']
element_list = ['C']
...
for element in element_list:
    ...
    for star_name in star_list:
        ...
        final_abund_file = np.genfromtxt('cra2_abund_error_table_ions.txt', dtype=None, encoding=None, names=True)

        for err_param in ['teff', 'logg', 'vmicro', 'mh']:
            ...
            star_teff = star_param['teff']
            star_logg = star_param['logg']
            star_vmicro = star_param['vmicro']
            star_mh = star_param['feh']
            
            if err_param == 'teff':
                star_teff += error_file['teff_err'][error_file['star_name']==star][0]
            elif err_param == 'logg':
                star_logg += error_file['logg_err'][error_file['star_name']==star][0]
            elif err_param == 'vmicro':
                star_logg += error_file['vmicro_err'][error_file['star_name']==star][0]
            elif err_param == 'mh':
                print(final_abund_file['STAR'])
                print(final_abund_file['FEI_H_SIGMA'][final_abund_file['STAR']==star][0])
                star_mh += final_abund_file['FEI_H_SIGMA'][final_abund_file['STAR']==star][0]


            ll_helper = LinelistManager(molecules=mp_mols, turbospec_path='../../Turbospectrum2019', source='linemake')

            ex_turbosynth2 = initiate_turbosynth(star_teff, 
                                                 star_logg,
                                                 star_mh,
                                                 star_vmicro,
                                                 star_param['cfe'],
            ...
            if not os.path.isdir(f'{star}/{err_param}_plus_err'):
                os.mkdir(f'{star}/{err_param}_plus_err')


            outfile = open(f'{star}/{err_param}_plus_err/{star}_{element}_abund_fit_convol{res:.1f}.txt', 'w')
            pp = PdfPages(f'{star}/{err_param}_plus_err/{star}_{element}_abund_fit_plots_convol{res:.1f}.pdf')
