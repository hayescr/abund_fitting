...
data = np.genfromtxt('gfallvac08oct17_300-1100.dat.txt', encoding=None,
                     dtype=None, names=names, delimiter=widths)


data = data[data['ion'] < 2.5]

ev_in_cm1 = 8065.5410

# Convert wavelengths from nm to angstroms and convert to air
data['wavelength'] *= 10.
data['wavelength'] = air_conversion(data['wavelength'])
...
no_gamma_rad = data['gamma_rad'] == 0.
data['gamma_rad'] = 10**data['gamma_rad']
# data['gamma_rad'][no_gamma_rad] = 0.0
data['gamma_rad'][no_gamma_rad] = 1.0e5

# change the missing gamma_vdw to 2.5 which seems to follow what is typical of TS
no_gamma_vdw = data['gamma_vdw'] == 0.
data['gamma_vdw'][no_gamma_vdw] = 2.5
...
with open('kurucz_gfallvac08oct17_300-1100.atoms.turbo.air', 'w') as outfile:
