...
def initiate_turbosynth(teff, logg, feh, vmicro, cfe, nfe, alphafe):

    solar_abund = au.solar_abund()
    abundances = {'N': solar_abund['N'] + feh + nfe, 'C': solar_abund['C'] + feh + cfe, }
    ...
    test = TurboSynth(path='tmp', turbopath='../../Turbospectrum2019')
...
for star_name in star_list:
    ...
    #ex_turbosynth2.set_abund('Eu', ex_turbosynth2.get_abund('Eu')+1.74)
    ex_turbosynth2.set_abund('Eu', ex_turbosynth2.get_abund('Eu')+1.6)
    ...
    element = 'Fe'
    lines = np.array(abund_lines[element]['convol'])
    ...
    converged_convol = False

    for i in range(convol_iterations):
        ...
        outfile = open(f'{star}/{star}_{element}_abund_fit_convol{res:.1f}.txt', 'w')
        pp = PdfPages(f'{star}/{star}_{element}_abund_fit_plots_convol{res:.1f}.pdf')

        print('# wavelength snr npix avg_line_depth avg_synth_stdev eqw eqw_err',
        ...
        #    try:
                ...
                print(f'{line:.2f} {abund_fitter.snr:.2f} {fit_range_pix}',
        ...
        good_lines &= abund_file['avg_synth_stdev']/(1.5/(abund_file['snr']*np.sqrt(abund_file['npix']))) > 1.0
        good_lines &= (abund_file['avg_line_depth'] * abund_file['snr'] > 5.)
        
        if len(abund_file[good_lines]) < 3:
            converged_convol = True
        else:
            abund_diff = abund_file['logeps_eqw'][good_lines] - abund_file['logeps_chi2'][good_lines]
            med_diff = np.nanmedian(abund_diff)
            sigma_diff = np.nanstd(abund_diff)/len(abund_diff)
            if ((med_diff < sigma_diff) & (med_diff > -sigma_diff)):
                converged_convol = True
            else:
                converged_convol = False
                res = res + med_diff*10.
