...
#element = 'N'
#lines = np.array(abund_lines[element]['CN'])
#lines = lines[lines > wavelength_limit]
#synth_range = 70

#element = 'Mg'
#lines = np.array(abund_lines[element]['MgH'])
#lines = lines[lines > wavelength_limit]
#synth_range = 5

#element = 'C'
#lines = np.array(abund_lines[element]['C13'])
#lines = lines[lines > wavelength_limit]
#synth_range = 5
...
for star_name in star_list:
    ...
    if star_name=='ret2_test':
        if 'Fe' in star_param['abundances']:
            fe_scale = star_param['abundances']['Fe'] - solar_abundances['Fe']
        else:
            fe_scale = star_param['feh']
        heavy_alphas = ['Si', 'S', 'Ar', 'Ca', 'Ti']
        for alpha in heavy_alphas:
            if (alpha not in star_param['abundances']):
                ex_turbosynth2.set_abund(alpha, fe_scale + 0.4 + solar_abundances[alpha])
        non_enhanced_elems = ['Sc','V', 'Cr', 'Mn', 'Co', 'Ni', 'Cu', 'Zn', 'Ga', 'Ge']
        for non_enh in non_enhanced_elems:
            if (non_enh not in star_param['abundances']):
                ex_turbosynth2.set_abund(non_enh, fe_scale + solar_abundances[non_enh])
    ...
    for line in lines:
        ...
        try:
            ...
            abund_fitter = Abundometer(ex_turbosynth2, mem, element, line,
                                       ...
                                       delta_abunds=np.arange(-0.6, 0.61, 0.3)
                                    )
